...
import { AnalysisResult } from './LLMService'

interface KBServiceInterface {
...
<body>
  ...
  async generateReport(
    ...
    htmlContent = htmlContent.replace('{{#if success}}Successful{{else}}Failed{{/if}}',
      analysisResult.success ? 'Successful' : 'Failed')

    // Replace each blocks
    htmlContent = this.replaceEachBlock(htmlContent, 'keyIssues', templateData.keyIssues, (item) =>
      `<li class="issue-item"><h3>${item}</h3></li>`
    )
    ...
    htmlContent = this.replaceEachBlock(htmlContent, 'recommendations', templateData.recommendations, (item) =>
  ...
  private replaceEachBlock(html: string, blockName: string, items: any[], itemTemplate: (item: any) => string): string {
  ...
  async getReports(reportsPath: string): Promise<Report[]> {
    if (!fs.existsSync(reportsPath)) {
      return []
    }
  ...
  async getReport(reportId: string, reportsPath: string): Promise<Report | null> {
  ...
  async importToCaseKB(
    report: Report,
    caseName: string,
    casesPath: string,
    kbService: KBServiceInterface
  ): Promise<void> {
